...
const splitFoods = (recipe) => {
    ...
    // Solution code here...
    for(let i = 0; i < recipe.ingredients.length; i++){
      let arrWords = recipe.ingredients[i].split(' ');
      let builtString = '';
      for(let j = 2; j < arrWords.length; j++){
        builtString = builtString + arrWords[j];
...
const stepActions = (recipe) => {
    let result = [];
    // Solution code here...
    //Isn't this simpler than the challenge above?
    for(let i = 0; i < recipe.steps.length; i++){
      let arrWords = recipe.steps[i].split(' ');
      result.push(arrWords[0]); 
    }
    return result;
  }
